...
import { postRegister } from "../slice.js";
...
function FormRegister() {
    ...
    useEffect(() => {
        setValidations({
            ...
            identical: password.length > 0 && password === passwordCheck,
                ...
                <ul className="text-white text-center m-3">
                    {Object.entries(validations).map(([key, isValid]) => (
                        <li key={key} id={key} className={isValid ? 'text-green-500' : 'text-red-500'}>
                            {key === 'identical' && 'Les mots de passe sont identicals.'}
                            {key === 'email' && 'Le mail a pour forme name@adresse.com  '}
                            {key === 'length' && 'Strictement plus de 8 caractères.'}
                            {key === 'uppercase' && 'Au moins une uppercase.'}
                            {key === 'number' && 'Au moins un number.'}
                            {key === 'specialCharacter' && 'Au moins un caractère spécial.'}
                        </li>
                    ))}
                </ul>
                <Button
